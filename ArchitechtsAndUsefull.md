# Архитектура · AbortController · Middleware · Нейминг файлов
...
Архитектурный подход: приложение разбивается на **независимо разрабатываемые и деплоящиеся части**, каждая из которых принадлежит отдельной команде. Аналог микросервисов, но для фронтенда.

```
Shell (Host App)
├── AuthApp      → команда A, React 18, CI/CD-пайплайн A
├── DashboardApp → команда B, Vue 3,  CI/CD-пайплайн B
└── CheckoutApp  → команда C, React 18, CI/CD-пайплайн C
```
...
| Подход | Суть | Когда |
|---|---|---|
| **Module Federation** | Webpack/Vite шарит JS-модули в рантайме | Основной выбор для SPA |
| **iframes** | Изоляция через встроенный фрейм | Полная изоляция важнее UX |
| **Web Components** | Custom Elements — фреймворко-независимо | Разные фреймворки в командах |
| **npm-пакеты** | Сборка во время CI, не в рантайме | Простота > независимые деплои |

---

#### Module Federation (Webpack 5) — полная конфигурация

```js
// remote/webpack.config.js — приложение, которое ОТДАЁТ модули
...
// host/webpack.config.js — приложение, которое ПОТРЕБЛЯЕТ модули
new ModuleFederationPlugin({
  name: "host",
  remotes: {
...
```tsx
// host/src/App.tsx — ленивая загрузка удалённого компонента
...
> `singleton: true` — критично для React/контекстов: гарантирует одну копию в рантайме.
...
```ts
// 1. CustomEvent — браузерный eventbus (простой вариант)
// remote: отправляет событие
...
// 2. Shared store через singleton (Zustand, Jotai)
// Работает если store вынесен в shared и помечен как singleton в ModuleFederation

// 3. Props / callbacks — если host рендерит remote напрямую
<LoginForm onSuccess={(user) => hostStore.setUser(user)} />
```
...
**Плюсы:**
- Независимые деплои — команды не блокируют друг друга
- Технологический стек на выбор команды (React, Vue, Svelte)
- Изолированные области ответственности и кодовые базы

**Минусы:**
- Сложность инфраструктуры (несколько CI/CD, coordination)
- Дублирование зависимостей если `shared` настроен неверно
- Первая загрузка медленнее (несколько `remoteEntry.js`)
- Сложный DX: отладка across приложений, TypeScript типы через границы
...
```
✅ Используй micro-frontend если:
   — 3+ независимые команды на одном продукте
   — Команды хотят деплоить независимо
   — Части приложения различаются технологически

❌ Избегай если:
   — Одна команда — сложность без выгоды
   — Нужен быстрый старт — оверхед на setup
   — Строгая консистентность UI — сложнее поддерживать design system
```

> **Альтернатива для Vite:** плагин `@originjs/vite-plugin-federation` реализует Module Federation без Webpack.
