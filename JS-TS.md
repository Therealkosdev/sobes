...
JavaScript никогда не блокирует поток, потому что он однопоточный, но неблокирующий.
Всё «волшебство» — в том, что всё тяжёлое (I/O, таймеры, события) вынесено за пределы JS-движка в отдельные потоки C++ (libuv в Node.js или браузерный движок).
